  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      try {
        ...
        // Skip token generation if no date/time
        if (!newBooking.date || !newBooking.time) {
          console.log("Skipping token - missing date or time");
        } else {
          // Generate secure token
          const token = crypto.randomUUID();

          // Use the original date/time from request body instead of DB result
          // (DB result might have different format)
          ...
          if (!isNaN(bookingDateTime.getTime())) {
            ...
            console.log(`Token generated for booking ${newBooking.id}`);
          } else {
            console.log("Invalid date format, skipping token");
          }
        }
      } catch (tokenError) {
