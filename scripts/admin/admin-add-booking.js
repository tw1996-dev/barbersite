...
export function setupAddBookingSection() {
    const serviceCheckboxes = document.querySelectorAll('input[name="admin-services"]');
    ...
    const statusGroup = document.getElementById('booking-status')?.closest('.form-group');
    if (statusGroup) {
...
function setupAddBookingCalendar() {
    ...
    const dateGroup = document.getElementById('booking-date')?.closest('.form-group');
    if (dateGroup) {
        dateGroup.innerHTML = `
    ...
    const timeGroup = document.getElementById('booking-time')?.closest('.form-group');
    if (timeGroup) {
...
export function renderAddBookingCalendar() {
    ...
    const selectedServices = document.querySelectorAll('input[name="admin-services"]:checked');
    let totalDuration = 0;
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const dayDate = new Date(addBookingCalendarYear, addBookingCalendarMonth, day);
        dayDate.setHours(0, 0, 0, 0);
        
        const dayDateStr = `${addBookingCalendarYear}-${String(addBookingCalendarMonth + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
        ...
        if (dayDate < today) {
            ...
            const hasAvailableSlots = totalDuration > 0 ? checkDayAvailability(dayDateStr, totalDuration, businessHours, currentBookings) : false;
            
            if (totalDuration > 0 && !hasAvailableSlots) {
                dayElement.classList.add('unavailable');
            } else if (totalDuration > 0) {
                dayElement.addEventListener('click', () => selectAddBookingDate(dayDateStr, dayElement));
            }
        }
...
function validateAddBookingForm() {
    ...
    const isValid = customerName && customerPhone && selectedServices.length > 0 && 
                   selectedAddBookingDate && selectedTime;
...
async function saveNewBooking() {
    ...
    const selectedServices = Array.from(document.querySelectorAll('input[name="admin-services"]:checked'));
    
    // Validation
    if (!form.customer || !form.phone || !form.date || !form.time || selectedServices.length === 0) {
    ...
    const totalPrice = selectedServices.reduce((sum, checkbox) => 
        sum + parseInt(checkbox.getAttribute('data-price')), 0);
