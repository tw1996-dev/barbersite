...
export function isTimeSlotAvailable(selectedDate, timeSlot, newDuration, currentBookings) {
    ...
    for (const booking of dayBookings) {
        ...
        if (newStartMinutes < existingNextAvailableMinutes && 
...
export function handleServiceConflicts(triggerId, checkboxSelector) {
    ...
    conflictingServices.forEach(conflictId => {
        const conflictCheckbox = document.querySelector(`${checkboxSelector}[value="${conflictId}"]`);
...
export function checkDayAvailability(dateStr, duration, businessHours, currentBookings) {
...
export function showNotification(message, type = 'info') {
    ...
    switch(type) {
        ...
        case 'error':
            ...
            break;
        case 'info':
            notification.style.background = 'linear-gradient(135deg, #4a9eff, #0066cc)';
            break;
        default:
